  ...
  useEffect(() => {
    ...
    function build() {
      ...
      for (let i = 0; i < COUNT; i++) {
        ...
        const angle =
          (stream / STREAMS) * Math.PI * 2 + (Math.random() - 0.5) * JITTER
    ...
    function animate(now: number) {
      ...
      for (const c of chars) {
        c.angle +=
          (ANGULAR_VEL * dt) / (c.radius / (maxR * 0.3) + 0.05) * scale
        ...
        if (c.radius <= CELL) {
          ...
          c.angle =
            (stream / STREAMS) * Math.PI * 2 +
            (Math.random() - 0.5) * JITTER
          c.el.textContent = rand()
        }

        const nc = Math.round(
          cx / CELL + Math.cos(c.angle) * c.radius / CELL,
        )
        const nr = Math.round(
          cy / CELL + Math.sin(c.angle) * c.radius / CELL,
        )
